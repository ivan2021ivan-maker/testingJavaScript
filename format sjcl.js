...
sjcl.cipher.aes.prototype = {
    ...
    O: function() {
        ...
        for (f = g = 0; !c[f]; f ^= l || 1, g = k[g] || 1)
            for (m = g ^ g << 1 ^ g << 2 ^ g << 3 ^ g << 4, m = m >> 8 ^ m & 255 ^ 99, c[f] = m, d[m] = f, n = h[e = h[l = h[f]]], p = 0x1010101 * n ^ 0x10001 * e ^ 0x101 * l ^ 0x1010100 * f, n = 0x101 * h[m] ^ 0x1010100 * m, e = 0; 4 > e; e++) a[e][f] = n = n << 24 ^ n >>> 8, b[e][m] = p = p << 24 ^ p >>> 8;
        for (e =
...
sjcl.hash.sha256.prototype = {
    ...
    update: function(a) {
        ...
        if ("undefined" !== typeof Uint32Array) {
            ...
            for (b = 512 + b - (512 + b & 0x1ff); b <= a; b += 512) u(this, c.splice(0, 16));
        return this
    },
    finalize: function() {
        var a, b = this.A,
...
sjcl.mode.ccm = {
    ...
    na: function(a, b, c, d, e, f) {
        ...
        d = a.encrypt(d);
        if (b.length)
            for (c = h.bitLength(b) / 8, 65279 >= c ? g = [h.partial(16, c)] : 0xffffffff >= c && (g = h.concat([h.partial(16, 65534)], [c])), g = h.concat(g, b), b = 0; b < g.length; b += 4) d = a.encrypt(k(d, g.slice(b, b + 4).concat([0, 0, 0])));
        return d
    },
...
sjcl.mode.ocb2 = {
    ...
    decrypt: function(a, b, c, d, e, f) {
        ...
        var g = sjcl.mode.ocb2.S,
            ...
            l = [0, 0, 0, 0],
            n = g(a.encrypt(c)),
            m, p, r = sjcl.bitArray.bitLength(b) - e,
            q = [];
        d = d || [];
